...
class SQLAccessor:
    ...
    def most_recent_garage_records(self) -> list[Garage]:
        stmt = select(GarageInfo.garage_name, GarageFullness.utc_timestamp, GarageFullness.fullness).join(GarageInfo).order_by(GarageFullness.utc_timestamp.desc()).limit(self.NUM_OF_GARAGES)
        with Session(self.engine) as sess:
            results = sess.execute(stmt)
            garages = []
            for name, timestamp, fullness in results:
                garages.append(Garage(name=name, utc_timestamp=timestamp, fullness=fullness))
            return garages
